...
public class ElasticSearchBulkDialog extends BaseStepDialog implements StepDialogInterface {
	...
	*/
	...
	private void fillIndexGroup(Composite parentTab) {
		...
		// Index
		wIndex = new LabelTextVar(transMeta, wIndexGroup,
		...
		// Type
		wType = new LabelTextVar(transMeta, wIndexGroup,
	...
	private void addServersTab() {
		...
		columnsMeta[0] = new ColumnInfo(
				BaseMessages.getString(PKG, "ElasticSearchBulkDialog.ServersTab.Address.Column"),
				ColumnInfo.COLUMN_TYPE_TEXT, false);
		columnsMeta[1] = new ColumnInfo(BaseMessages.getString(PKG, "ElasticSearchBulkDialog.ServersTab.Port.Column"),
				ColumnInfo.COLUMN_TYPE_TEXT, true);

		wServers = new TableView(transMeta, wServersComp, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI, columnsMeta, 1,
				lsMod, props);
	...
	private void addSettingsTab() {
		...
		columnsMeta[0] = new ColumnInfo(
				...
				ColumnInfo.COLUMN_TYPE_TEXT, false);
		columnsMeta[1] = new ColumnInfo(BaseMessages.getString(PKG, "ElasticSearchBulkDialog.SettingsTab.Value.Column"),
				ColumnInfo.COLUMN_TYPE_TEXT, false);

		wSettings = new TableView(transMeta, wSettingsComp, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI, columnsMeta, 1,
				lsMod, props);
		FormData fdServers = new FormData();
		fdServers.left = new FormAttachment(0, Const.MARGIN);
		fdServers.top = new FormAttachment(0, Const.MARGIN);
	...
	private void addFieldsTab() {
		...
		ColumnInfo[] columnsMeta = new ColumnInfo[2];
		columnsMeta[0] = new ColumnInfo(BaseMessages.getString(PKG, "ElasticSearchBulkDialog.NameColumn.Column"),
				ColumnInfo.COLUMN_TYPE_CCOMBO, names, false);
		columnsMeta[1] = new ColumnInfo(BaseMessages.getString(PKG, "ElasticSearchBulkDialog.TargetNameColumn.Column"),
				ColumnInfo.COLUMN_TYPE_TEXT, false);

		wFields = new TableView(transMeta, wFieldsComp, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI, columnsMeta,
	...
	public void getData(ElasticSearchBulkMeta in) {
		...
		wJsonField.setEnabled(wIsJson.getSelection()); // listener not working
														// here

		wUseOutput.setSelection(in.isUseOutput());

		wIdOutField.setText(Const.NVL(in.getIdOutField(), ""));
		wIdOutField.setEnabled(wUseOutput.getSelection()); // listener not
															// working here
	...
	private void toModel(ElasticSearchBulkMeta in) throws KettleException { // copy
																			// info
																			// to
	...
	private void test(TestType testType) {
		...
		// Now ensure that the thread's context class loader is the plugin's
		// classloader
		...
		try {
			...
			if (tempMeta.getSettingsMap().containsKey("xpack.security.user")) {
				logBasic("Setting up parameter for X-Pack");
				Set<String> keys = tempMeta.getSettingsMap().keySet();
				Builder builder = Settings.builder();
				for (String key : keys) {
					logBasic("Setting " + key + ": " + tempMeta.getSettingsMap().get(key));
					builder.put(key, tempMeta.getSettingsMap().get(key));
				}
				logBasic(builder.keys().toString());
				settings = builder.build();
				tClientBuilder = new PreBuiltXPackTransportClient(settings);
			} else {
				logBasic("Setting up parameter for without X-Pack");
				Set<String> keys = tempMeta.getSettingsMap().keySet();
				Builder builder = null;
				for (String key : keys) {
					logBasic("Setting " + key + ": " + tempMeta.getSettingsMap().get(key));
					builder = Settings.builder().put(key, tempMeta.getSettingsMap().get(key));
				}
				settings = builder.build();
				tClientBuilder = new PreBuiltTransportClient(settings);
			}

			if (!tempMeta.getServers().isEmpty()) {
				node = null;
				TransportClient tClient = tClientBuilder;
